...
# pobierz zbiór obrazów Oxford-IIIT Pets
...
train_images: _ParallelMapDataset = dataset['train'].map(load_image, num_parallel_calls=tf.data.AUTOTUNE)
test_images: _ParallelMapDataset = dataset['test'].map(load_image, num_parallel_calls=tf.data.AUTOTUNE)
...
train_batches: _PrefetchDataset = (
    train_images
    .cache()
    .shuffle(BUFFER_SIZE)
    .batch(BATCH_SIZE)
...
test_batches: _BatchDataset = test_images.batch(BATCH_SIZE)
